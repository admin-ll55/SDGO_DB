...
function auto_correct(e, s) {
  if (e.prev().length && s != "s") {
    e.prev().children('input').prop('checked', false);
  }
  if (e.next().length && s != "s") {
    e.next().children('input').prop('checked', false);
  }
  if (e.next().next().length && s == "s") {
    e.next().next().children('input').prop('checked', false);
  }
  if (e.prev().length && e.next().length && s == "s") {
    e.next().children('input').prop('checked', false);
  }
  if (e.prev().prev().length && s == "s") {
    e.prev().children('input').prop('checked', false);
    e.prev().prev().children('input').prop('checked', false);
  }
}
function resetform(){
...
function form_submit(form, key, value) {
  form.children("input[name='"+key+"']").val(value);
  $.get("search_v2?"+key+"="+value, function() {
