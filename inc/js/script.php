<?php
$sql = "SELECT `skl`, `name_".$_COOKIE["l"]."`, `desc_".$_COOKIE["l"]."` FROM `skill`;";
...
if ($result->rowCount() >= 1) {
  while($row = $result->fetch()) {
    if ($row["skl"] != "") {
      ...
      $temp[0] .= "'".$row["skl"]."'";
      $temp[1] .= "'".$row["name_".$_COOKIE["l"]]."'";
      $temp[2] .= "'".$row["desc_".$_COOKIE["l"]]."'";
    }
  }
}
...
function ch_l(l) {
  ...
  var eff = [
    ...
    ["狙击","扩散","缓速","追尾","修理","牵引","抽气","贯通","瞬移","蓄力","倒地","隐形","无格冲","7秒I力场","发射硬直","枪械故障","装填下降","一时硬直","迎击模式","突进格斗","鼠标反转","全方位攻撃","格斗反击无效","6秒内推进量增加","弹药减少不可变形","雷达显示命中机体","判定依射击武器决定","7秒内防御力大幅增加","7秒内受到光束的伤害-70%"]
  ];
