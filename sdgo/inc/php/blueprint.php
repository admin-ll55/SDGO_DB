...
if ($unit_name = API::call(["type"=>"unit_name","data"=>["id"=>$id]])) {
  ...
  foreach ($unique as $v) {
    ...
    if ($ids[0] == $id) {
      $meta_description = "{$unit_name} ".tos("設計圖", "设计图").": {$unit_name_a}[Lv. {$blueprint["aa"]}], {$unit_name_b}[Lv. {$blueprint["bb"]}], {$unit_name_c}[Lv. {$blueprint["cc"]}], {$unit_name_d}[Lv. {$blueprint["dd"]}], {$unit_name_e}[Lv. {$blueprint["ee"]}]";
